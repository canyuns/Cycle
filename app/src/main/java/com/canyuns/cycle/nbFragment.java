...
public class nbFragment extends ListFragment{
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,  
        ...
        for (int i = 0; i < routeStrings.TITLES_nb.length; i++) {
            Map<String, Object> item = new HashMap<>();
            item.put("image", routeStrings.ICONS_nb[i]);
            item.put("text", routeStrings.TITLES_nb[i]);
            items.add(item);
        }
    ...
    void showDetails(int index) {
            ...
            intent.putExtra("index", index+routeStrings.TITLES.length + routeStrings.TITLES_sx.length);
